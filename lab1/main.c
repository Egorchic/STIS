...
int main() {
    ...
    Frame frame_create_dataset, frame_use_dataset;
    initFrame(&frame_create_dataset, "СОЗДАТЬ.ДАТАСЕТ");
    initFrame(&frame_use_dataset, "ИСПОЛЬЗОВАТЬ.ДАТАСЕТ");
    ...
    printf("\n");
    printFrame(&frame_use_dataset, concepts);
